...
#install.packages(c("shiny", "shinydashboard", "plotly", "tidyverse", "readxl", "DT", "shinycssloaders", "ComplexUpset"))
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    padding-top: 80px !important;  /* 关键：让内容避开 header */
  ...
  ')),
    tabsetPanel(type = "tabs",
                tabPanel("ThermoTargetMiner",
                         ...
                         h3("GO Enrichment Analysis (Non-zero OPLS‑DA Genes)"),
                         uiOutput("go_status"),
...
server <- function(input, output, session) {
  ...
  output$scatter1 <- renderPlotly({
    ...
    # 合并原始OPLS-DA数据和transformed数据的对应列
    df_joined <- df %>%
      left_join(
    ...
    # >4.5基因用transformed数据判断
    genes_over_4.5 <- df_joined %>%
      filter(!is.na(.data[[value_col]]), .data[[value_col]] > 4.5) %>%
      pull(`Gene names`)
    
    # top10基因颜色根据OPLS-DA的pq列正负决定
    df_joined <- df_joined %>%
      mutate(
        gene_type = case_when(
          ...
          `Gene names` %in% genes_over_4.5 ~ "over4.5",
          TRUE ~ "other"
        )
      )
  ...
  output$scatter2 <- renderPlotly({
    ...
    opls_joined <- opls_df %>%
      dplyr::left_join(trans_sub, by = "Gene names")
    
    genes_over_4.5 <- opls_joined %>%
      dplyr::filter(!is.na(.data[[value_col]]), .data[[value_col]] > 4.5) %>%
  ...
  output$plot_go_bp <- renderPlot({
    ...
    if (nrow(res) == 0) {
      plot.new()
      title("No enriched GO Biological Process terms")
    } else {
      dotplot(res, showCategory = 10, title = "GO Biological Process")
    }
  })
  ...
  output$plot_go_cc <- renderPlot({
    ...
    if (nrow(res) == 0) {
      plot.new()
      title("No enriched GO Cellular Component terms")
    } else {
      dotplot(res, showCategory = 10, title = "GO Cellular Component")
    }
  })
  ...
  output$plot_go_mf <- renderPlot({
    ...
    if (nrow(res) == 0) {
      plot.new()
      title("No enriched GO Molecular Function terms")
    } else {
      dotplot(res, showCategory = 10, title = "GO Molecular Function")
    }
  })
  ...
  output$bar_chart <- renderPlotly({
    ...
    colors <- ifelse(values < 4.5, "#B0B0B0", "#637A9F")
  ...
  upset_data <- reactive({
    ...
    # original data for table presentation
    df_raw <- df %>%
    ...
    # count k
    df_raw$Count_Above_4.5 <- rowSums(df_raw[, group_cols_new] > 4.5, na.rm = TRUE)
    
    # logic data for upset plot
    df_logic <- df_raw %>%
      mutate(across(all_of(group_cols_new), ~ . > 4.5)) %>%
  ...
  output$protarget_table <- DT::renderDT({
    ...
    # remove Gene_name column，only show rows with Count_Above_4.5 > 1 
    df_filtered <- df_raw %>%
      dplyr::select(-Gene_name) %>%
      dplyr::filter(Count_Above_4.5 > 1)
